...
public class WearNotificationsTest {
    ...
    private void disableArchiveAction() {
        when(account.getArchiveFolderName()).thenReturn(PerfectMail.FOLDER_NONE);
    }

    private void disableSpamAction() {
        when(account.getSpamFolderName()).thenReturn(PerfectMail.FOLDER_NONE);
    }

    private void enableDeleteAction() {
        PerfectMail.setNotificationQuickDeleteBehaviour(NotificationQuickDelete.ALWAYS);
        PerfectMail.setConfirmDeleteFromNotification(false);
    }
