...
class WearNotifications extends BaseNotifications {
    ...
    private boolean isDeleteActionAvailableForWear() {
        return isDeleteActionEnabled() && !PerfectMail.confirmDeleteFromNotification();
    }
    ...
    private boolean isSpamActionAvailableForWear(Account account) {
        ...
        return spamFolderName != null && !PerfectMail.confirmSpam() && isMovePossible(account, spamFolderName);
    }

    private boolean isMovePossible(Account account, String destinationFolderName) {
        if (PerfectMail.FOLDER_NONE.equalsIgnoreCase(destinationFolderName)) {
            return false;
        }
