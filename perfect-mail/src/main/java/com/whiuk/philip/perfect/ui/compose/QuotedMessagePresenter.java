package com.whiuk.philip.perfect.ui.compose;
...
import com.whiuk.philip.perfect.Account.QuoteStyle;
import com.whiuk.philip.perfect.activity.MessageCompose;
import com.whiuk.philip.perfect.activity.MessageCompose.Action;
import com.whiuk.philip.perfect.message.extractors.BodyTextExtractor;
import com.whiuk.philip.perfect.message.html.HtmlConverter;
import com.whiuk.philip.perfect.message.quote.HtmlQuoteCreator;
import com.whiuk.philip.perfect.message.quote.TextQuoteCreator;
import com.whiuk.philip.perfect.message.signature.HtmlSignatureRemover;
...
import com.whiuk.philip.perfect.mailstore.AttachmentResolver;
import com.whiuk.philip.perfect.mailstore.MessageViewInfo;
import com.whiuk.philip.perfect.message.IdentityField;
import com.whiuk.philip.perfect.message.quote.InsertableHtmlContent;
...
import com.whiuk.philip.perfect.message.signature.TextSignatureRemover;
...
public class QuotedMessagePresenter {
    ...
    public void processDraftMessage(MessageViewInfo messageViewInfo, Map<IdentityField, String> identity) {
        quoteStyle = identity.get(IdentityField.QUOTE_STYLE) != null
                ? QuoteStyle.valueOf(identity.get(IdentityField.QUOTE_STYLE))
                : account.getQuoteStyle();

        int cursorPosition = 0;
        if (identity.containsKey(IdentityField.CURSOR_POSITION)) {
            try {
                cursorPosition = Integer.parseInt(identity.get(IdentityField.CURSOR_POSITION));
            } catch (Exception e) {
        ...
        if (identity.containsKey(IdentityField.QUOTED_TEXT_MODE)) {
            showQuotedTextMode = identity.get(IdentityField.QUOTED_TEXT_MODE);
        } else {
            showQuotedTextMode = "NONE";
        }

        int bodyLength = identity.get(IdentityField.LENGTH) != null ?
                Integer.valueOf(identity.get(IdentityField.LENGTH)) : UNKNOWN_LENGTH;
        int bodyOffset = identity.get(IdentityField.OFFSET) != null ?
                Integer.valueOf(identity.get(IdentityField.OFFSET)) : UNKNOWN_LENGTH;
        Integer bodyFooterOffset = identity.get(IdentityField.FOOTER_OFFSET) != null ?
                Integer.valueOf(identity.get(IdentityField.FOOTER_OFFSET)) : null;
        Integer bodyPlainLength = identity.get(IdentityField.PLAIN_LENGTH) != null ?
                Integer.valueOf(identity.get(IdentityField.PLAIN_LENGTH)) : null;
        Integer bodyPlainOffset = identity.get(IdentityField.PLAIN_OFFSET) != null ?
                Integer.valueOf(identity.get(IdentityField.PLAIN_OFFSET)) : null;
        ...
        String messageFormatString = identity.get(IdentityField.MESSAGE_FORMAT);
