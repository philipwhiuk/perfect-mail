...
import com.whiuk.philip.perfect.Account.SortType;
import com.whiuk.philip.perfect.activity.MessageCompose;
import com.whiuk.philip.perfect.activity.UpgradeDatabases;
...
import com.whiuk.philip.perfect.mail.internet.BinaryTempFileBody;
import com.whiuk.philip.perfect.mail.ssl.LocalKeyStore;
import com.whiuk.philip.perfect.mailstore.LocalStore;
import com.whiuk.philip.perfect.power.DeviceIdleManager;
import com.whiuk.philip.perfect.preferences.Storage;
import com.whiuk.philip.perfect.preferences.StorageEditor;
import com.whiuk.philip.perfect.provider.UnreadWidgetProvider;
import com.whiuk.philip.perfect.service.BootReceiver;
import com.whiuk.philip.perfect.service.MailService;
import com.whiuk.philip.perfect.service.ShutdownReceiver;
import com.whiuk.philip.perfect.service.StorageGoneReceiver;
import com.whiuk.philip.perfect.widget.list.MessageListWidgetProvider;
import timber.log.Timber;
import timber.log.Timber.DebugTree;


public class PerfectMail extends Application {
    /**
     * Components that are interested in knowing when the PerfectMail instance is
     * available and ready (Android invokes Application.onCreate() after other
     * components') should implement this interface and register using
     * {@link PerfectMail#registerApplicationAware(ApplicationAware)}.
     */
     ...
     * was called.
     * Afterwards calls to {@link #registerApplicationAware(PerfectMail.ApplicationAware)} will
     * immediately call {@link PerfectMail.ApplicationAware#initializeComponent(android.app.Application)} for the
     * supplied argument.
     */
    ...
    public static final String IDENTITY_HEADER = MailLibrary.IDENTITY_HEADER;
    ...
    public static final int PUSH_WAKE_LOCK_TIMEOUT = MailLibrary.PUSH_WAKE_LOCK_TIMEOUT;
    ...
    public static class Intents {
        ...
        public static class Share {
            /*
             * We don't want to use EmailReceived.EXTRA_FROM ("com.whiuk.philip.k9.intent.extra.FROM")
    ...
    public static void save(StorageEditor editor) {
        editor.putBoolean("enableDebugLogging", PerfectMail.DEBUG);
        editor.putBoolean("enableSensitiveLogging", PerfectMail.DEBUG_SENSITIVE);
        editor.putString("backgroundOperations", PerfectMail.backgroundOps.name());
    ...
    public void onCreate() {
        ...
        MailLibrary.setDebugStatus(new MailLibrary.DebugStatus() {
        ...
        MessagingController.getInstance(this).addListener(new SimpleMessagingListener() {
            private void broadcastIntent(String action, Account account, String folder, Message message) {
                ...
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_ACCOUNT, account.getDescription());
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_FOLDER, folder);
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_SENT_DATE, message.getSentDate());
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_FROM, Address.toString(message.getFrom()));
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_TO, Address.toString(message.getRecipients(Message.RecipientType.TO)));
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_CC, Address.toString(message.getRecipients(Message.RecipientType.CC)));
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_BCC, Address.toString(message.getRecipients(Message.RecipientType.BCC)));
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_SUBJECT, message.getSubject());
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_FROM_SELF, account.isAnIdentity(message.getFrom()));
                PerfectMail.this.sendBroadcast(intent);
            ...
            private void updateUnreadWidget() {
                try {
                    UnreadWidgetProvider.updateUnreadCount(PerfectMail.this);
                } catch (Exception e) {
            ...
            private void updateMailListWidget() {
                try {
                    MessageListWidgetProvider.triggerMessageListWidgetUpdate(PerfectMail.this);
            ...
            public void synchronizeMailboxRemovedMessage(Account account, String folder, Message message) {
                broadcastIntent(PerfectMail.Intents.EmailReceived.ACTION_EMAIL_DELETED, account, folder, message);
                updateUnreadWidget();
            ...
            public void synchronizeMailboxNewMessage(Account account, String folder, Message message) {
                broadcastIntent(PerfectMail.Intents.EmailReceived.ACTION_EMAIL_RECEIVED, account, folder, message);
                updateUnreadWidget();
                updateMailListWidget();
            }
            ...
            public void folderStatusChanged(Account account, String folderName,
                ...
                Intent intent = new Intent(PerfectMail.Intents.EmailReceived.ACTION_REFRESH_OBSERVER, null);
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_ACCOUNT, account.getDescription());
                intent.putExtra(PerfectMail.Intents.EmailReceived.EXTRA_FOLDER, folderName);
                PerfectMail.this.sendBroadcast(intent);
    ...
    public void checkCachedDatabaseVersion() {
        ...
        if (cachedVersion >= LocalStore.DB_VERSION) {
            PerfectMail.setDatabasesUpToDate(false);
        }
    }
     ...
     * {@link com.whiuk.philip.perfect.preferences.GlobalSettings}, too.
     *
    ...
    public static void loadPrefs(Preferences prefs) {
        ...
        if (themeValue == Theme.DARK.ordinal() || themeValue == android.R.style.Theme) {
            PerfectMail.setK9Theme(Theme.DARK);
        } else {
            PerfectMail.setK9Theme(Theme.LIGHT);
        }

        themeValue = storage.getInt("messageViewTheme", Theme.USE_GLOBAL.ordinal());
        PerfectMail.setK9MessageViewThemeSetting(Theme.values()[themeValue]);
        themeValue = storage.getInt("messageComposeTheme", Theme.USE_GLOBAL.ordinal());
        PerfectMail.setK9ComposerThemeSetting(Theme.values()[themeValue]);
        PerfectMail.setUseFixedMessageViewTheme(storage.getBoolean("fixedMessageViewTheme", true));
    }
    ...
    /**
     * Register a component to be notified when the {@link PerfectMail} instance is ready.
     *
     * @param component
    ...
    public static void registerApplicationAware(final ApplicationAware component) {
        synchronized (observers) {
            if (sInitialized) {
                component.initializeComponent(PerfectMail.app);
    ...
    public static int getMailThemeResourceId(Theme themeId) {
        return (themeId == Theme.LIGHT) ? com.whiuk.philip.perfect.R.style.Theme_K9_Light : com.whiuk.philip.perfect.R.style.Theme_K9_Dark;
    }
    ...
    public static boolean setBackgroundOps(BACKGROUND_OPS backgroundOps) {
        BACKGROUND_OPS oldBackgroundOps = PerfectMail.backgroundOps;
        PerfectMail.backgroundOps = backgroundOps;
    ...
    public static void setAttachmentDefaultPath(String attachmentDefaultPath) {
        PerfectMail.mAttachmentDefaultPath = attachmentDefaultPath;
    }
    ...
    public static void setPgpInlineDialogCounter(int pgpInlineDialogCounter) {
        PerfectMail.sPgpInlineDialogCounter = pgpInlineDialogCounter;
    }
    ...
    public static void setPgpSignOnlyDialogCounter(int pgpSignOnlyDialogCounter) {
        PerfectMail.sPgpSignOnlyDialogCounter = pgpSignOnlyDialogCounter;
    }
