...
import com.whiuk.philip.perfect.activity.compose.ComposeCryptoStatus.AttachErrorState;
import com.whiuk.philip.perfect.activity.compose.ComposeCryptoStatus.ComposeCryptoStatusBuilder;
...
import com.whiuk.philip.perfect.helper.ReplyToParser.ReplyToAddresses;
...
import com.whiuk.philip.perfect.view.RecipientSelectView.Recipient;
...
public class RecipientPresenter implements PermissionPingCallback {
    ...
    public void onPrepareOptionsMenu(Menu menu) {
        ...
        boolean showSignOnly = isCryptoConfigured && PerfectMail.getOpenPgpSupportSignOnly();
    ...
    private void setupCryptoProvider() {
        String openPgpProvider = PerfectMail.getOpenPgpProvider();
    ...
    private boolean checkAndIncrementPgpInlineDialogCounter() {
        int pgpInlineDialogCounter = PerfectMail.getPgpInlineDialogCounter();
        if (pgpInlineDialogCounter < PGP_DIALOG_DISPLAY_THRESHOLD) {
            PerfectMail.setPgpInlineDialogCounter(pgpInlineDialogCounter + 1);
            PerfectMail.saveSettingsAsync();
            return true;
        }
        return false;
    }
    ...
    private boolean checkAndIncrementPgpSignOnlyDialogCounter() {
        int pgpSignOnlyDialogCounter = PerfectMail.getPgpSignOnlyDialogCounter();
        if (pgpSignOnlyDialogCounter < PGP_DIALOG_DISPLAY_THRESHOLD) {
            PerfectMail.setPgpSignOnlyDialogCounter(pgpSignOnlyDialogCounter + 1);
            PerfectMail.saveSettingsAsync();
            return true;
        }
        return false;
